...
DECLARE_CACHE(nand, bin_cache_entry);
#define NEGATE(f) ite(f,zero,one)

node *nand(node *f, node *g)
    {
	if (IS_TERMINAL(f)) {
	    return f == one ? NEGATE(g) : one;
	} else if (IS_TERMINAL(g)) {
	    return g == one ? NEGATE(f) : one;
	} else {
	    node *result;
	    DECLARE_BIN_CACHE_ENTRY

	    TRY_BIN_CACHE(f, g, nand);

	    if (f->value < g->value) {
		result = make_node(f->value, nand(f->tr, g), nand(f->fa, g));
	    } else if (f->value > g->value) {
		result = make_node(g->value, nand(f, g->tr), nand(f, g->fa));
	    } else /* f->value == g->value */{
		result = make_node(f->value,
				   nand(f->tr, g->tr),
				   nand(f->fa, g->fa));
	    }
	    UPDATE_BIN_CACHE(f, g, result, nand);
	    return result;
	}
    }
...
#define QUATERNARY_NODE_HASH(a,b,c,d) \
      ((INTCAST(a)+		\
	  (INTCAST(b)<<1)+	\
	  (INTCAST(b)<<2)+	\
	  (INTCAST(b)<<3)) % COMPUTED_TABLE_SIZE)
...
#define DECLARE_QUAT_CACHE_ENTRY quat_cache_entry *cache;

#define UPDATE_QUAT_CACHE(n1,n2,n3,n4,robdd,op) \
do {	\
	cache->a = n1; \
	cache->b = n2; \
	cache->c = n3; \
...
#define TRY_QUAT_CACHE(n1,n2,n3,n4,op)					\
do {									\
	cache = &((op##_computed_cache)[QUATERNARY_NODE_HASH(n1,n2,n3,n4)]);	\
	if (cache->a==(n1) && cache->b==(n2) && cache->c==(n3) && cache->d==(n4)) {				\
	    return cache->result;					\
	}								\
} while (0)

DECLARE_CACHE(and_or_and, quat_cache_entry);

// computes (f1 & f2) V (g1 & g2)
node *and_or_and(node *f1, node *f2, node *g1, node *g2) {
	if (f1 == zero || f2 == zero) {
		return glb(g1,g2);
	} else if (g1 == zero || g2 == zero){
		return glb(f1,f2);
	} else if ((f1 == one && f2 == one) || (g1 == one && g2 == one)) {
		return one;
	}
	DECLARE_QUAT_CACHE_ENTRY

	// None are zero nodes, at least one root in each pair is non-terminal

	// Need to canonicalise ordering of input nodes

	// sort f pair, putting non-terminal nodes in f2
	if (IS_TERMINAL(f1) || (!IS_TERMINAL(f2) && f2->value < f1->value)) {
		SWAP_PTRS(node,f1,f2);
	}
	// sort g pair, putting non-terminal nodes in g2
	if (IS_TERMINAL(g1) || (!IS_TERMINAL(g2) && g2->value < g1->value)) {
		SWAP_PTRS(node,g1,g2);
	}

	// sort f and g according to f1 and g1

	if (g1->value < f1->value) {
		SWAP_PTRS(node,f1,g1); SWAP_PTRS(node,f2,g2);
	}

	TRY_QUAT_CACHE(f1,f2,g1,g2, and_or_and);

	// f1 has the min_var
	int min_var = f1->value;

	node *tr_f2,*fa_f2, *tr_g1, *fa_g1, *tr_g2, *fa_g2;

	if (!IS_TERMINAL(f2) && f2->value == min_var) {
		tr_f2 = f2->tr;
		fa_f2 = f2->fa;
	} else {
		tr_f2 = f2;
	...
	if (!IS_TERMINAL(g1) && g1->value == min_var) {
		tr_g1 = g1->tr;
		fa_g1 = g1->fa;
	} else {
		tr_g1 = g1;
	...
	if (!IS_TERMINAL(g2) && g2->value == min_var) {
		tr_g2 = g2->tr;
		fa_g2 = g2->fa;
	} else {
		tr_g2 = g2;
	...
	node *result = make_node(min_var, and_or_and(f1->tr,tr_f2,tr_g1,tr_g2)
									 , and_or_and(f1->fa,fa_f2,fa_g1,fa_g2));

	UPDATE_QUAT_CACHE(f1,f2,g1,g2, result, and_or_and);
	return result;
}

DECLARE_CACHE(and_implies_and, quat_cache_entry);

// computes (f1 & f2) -> (g1 & g2)
node *and_implies_and(node *f1, node *f2, node *g1, node *g2) {

	// Canonicalise ordering of input nodes

	// sort f pair, putting non-terminal nodes in f1
	if ((IS_TERMINAL(f1) && f1 < f2) || (!IS_TERMINAL(f2) && f2->value < f1->value)) {
		SWAP_PTRS(node,f1,f2);
	}
	// sort g pair, putting non-terminal nodes in g1
	if ((IS_TERMINAL(g1) && g1 < g2)|| (!IS_TERMINAL(g2) && g2->value < g1->value)) {
		SWAP_PTRS(node,g1,g2);
	}

	if (f1 == one && f2 == one) return glb(g1,g2);
	if (f1 == zero || f2 == zero || (g1 == one && g2 == one)) return one;
	if (g1 == zero || g2 == zero) return nand(f1,f2);
	if (f2 == one && g2 == one ) return implies(f1, g1);

	DECLARE_QUAT_CACHE_ENTRY
	TRY_QUAT_CACHE(f1,f2,g1,g2, and_implies_and);

	assert(!IS_TERMINAL(f1) && !IS_TERMINAL(g1));
	int min_var = min(f1->value, g1->value);

	node *tr_f1,*fa_f1,*tr_f2,*fa_f2, *tr_g1, *fa_g1, *tr_g2, *fa_g2;
	...
	node *result = make_node(min_var, and_implies_and(tr_f1,tr_f2,tr_g1,tr_g2)
									 , and_implies_and(fa_f1,fa_f2,fa_g1,fa_g2));

	UPDATE_QUAT_CACHE(f1,f2,g1,g2, result, and_implies_and);
	return result;
}